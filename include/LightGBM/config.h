...
struct Config {
  ...
  */
  ...
  // alias = gradient_max
  // check = >=0.0
  // desc = absolute value of the max bound if > 0, no effect if ==0
  // desc = learning_rate is adjusted
  double gradient_bound = 0.0;

  // alias = num_leaves_initial
  // check = >1
  // check = <=131072
  // desc = max number of leaves in the first tree
  int num_leaves_initial = 131072;

  // alias = canarito, qcanaritos, canaries, canary
  // check = >=-1
